...
namespace satellites {
	...
	Client_Scanner::Client_Scanner(const vector<ChromosomeOneDigit*>* chromListIn,
		...
		double base, double idnIn, int mtfIn,
		Predictor<int> * predIn, int minRegIn) :
 	...
 	for (int i = 0; i < chromList->size(); i++) {
		...
		// Find the repeated motif in this STR
 		for (int j = 0; j < chromSats->size(); j++) {
			...
			// window
 			if (sat->getLength() > (minReg / 2.0)) {
 				if (mtf) {
 					FindMotif * findMotif;
 					string candidateSample;

 					candidateSample = candidate.substr(0, 20000);
 ...
 */
 ...
 void Client_Scanner::decode(vector<vector<ILocation*>*>& sats_set,
 	vector<ScorerSat*>* scorers,
 	const vector<ChromosomeOneDigit*>* chroms) {
 	for (int i = 0; i < scorers->size(); i++) {
 		ScorerSat* scorer = scorers->at(i);
 		vector<utility::ILocation*>* chromSats =
 		new vector<utility::ILocation*>(0);

 		for (auto& segment : *chroms->at(i)->getSegment()) {
 			int segStart = segment->at(0);
 			int segEnd = segment->at(1);
 			hmm->decode(segStart, segEnd, scorer->getFlatScores(), *chromSats);
 		}

 		sats_set.push_back(chromSats);
 	}
 }
